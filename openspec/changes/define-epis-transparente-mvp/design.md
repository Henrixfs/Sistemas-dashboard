...
| Migraciones | Alembic como única fuente de verdad del esquema financiero | Scripts SQL manuales, `init-db.sql` con tablas de negocio | Alembic versiona de forma reproducible los cambios de esquema junto con SQLAlchemy; PostgreSQL se prepara con `alembic upgrade head` y no tendrá un esquema financiero paralelo. |
...
| Almacenamiento de comprobantes | Sistema de archivos con referencia en BD, historial de reemplazos y acceso controlado por la aplicación | BD directa (BLOB), CDN externo | Almacenar binarios en BD escala mal; CDN es innecesario para el MVP. Un gasto requiere al menos un comprobante válido al publicarse, pero los borradores y los ingresos pueden existir sin comprobante. |
| Autenticación y roles | Sesión con cookie HttpOnly y token opaco hasheado; alumno inicia sesión con `codigo_estudiante`, administrador y superadministrador con email | Un único identificador por email, JWT sin estado, SSO institucional | Los alumnos se importan desde el padrón institucional; separar identificadores preserva ese flujo y permite cuentas administrativas sin código estudiantil. Los roles son alumno, administrador y superadministrador. |
| Contraseña inicial e identidad | Importación de alumnos con contraseña temporal derivada del DNI, hasheada inmediatamente, y cambio obligatorio al primer acceso | Persistir DNI o contraseña temporal, autoregistro | No se persiste el DNI como contraseña ni se registra en auditoría. El algoritmo de hash y el importador se implementarán en los bloques de autenticación e importación. |
| Administración privilegiada | Superadministrador bootstrap seguro, creación de administradores y restablecimiento administrativo | Credenciales hardcodeadas, administrador ordinario con privilegios de bootstrap | El bootstrap no incluye usuario ni contraseña en código, migraciones, Docker o archivos versionados; un administrador ordinario no crea superadministradores. |
| Auditoría | Registro en tabla separada (who, what, when, detail JSONB) con protección de BD contra UPDATE/DELETE | Logs de aplicación, solo triggers de BD | La tabla permite consultas y reportes; la protección de BD refuerza su inmutabilidad sin sustituir el registro explícito. |
...
| Integridad financiera | Constraints, FKs no destructivas, validación transaccional de publicación y triggers solo para reglas dependientes del estado previo o de varias tablas | Confiar únicamente en la aplicación | Las reglas declarativas protegen los datos básicos; los triggers cubren cierre de periodos, borrado histórico e inmutabilidad, y la publicación verifica atómicamente el comprobante de gastos. |
